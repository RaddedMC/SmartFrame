...
def GetCardData():
	...
	for chromecast in chromecasts:
		if chromecast.status.status_text == '':
			...
			itemList.append(Item("volume", device_icon, (52,115,196), bgFillAmt=volume_level))
			...
			if play_icon != None:
				if chromecast.media_controller.status.duration == None:
					...
					itemList.append(Item("play_pause", play_icon, (255,0,0), bgFillAmt=1.0))

				else:
					player_progress = chromecast.media_controller.status.current_time / chromecast.media_controller.status.duration
					itemList.append(Item("play_pause", play_icon, (152,50,196), bgFillAmt=player_progress))
